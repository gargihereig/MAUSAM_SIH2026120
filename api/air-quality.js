...
module.exports = async function airQuality(req, res) {
    ...
    try {
        ...
        if (!response.ok) return sendJson(res, 502, { error: 'Air quality service failed' });
        ...
        if (aqi === null) return sendJson(res, 502, { error: 'Air quality data unavailable' });
        ...
        console.error('[air-quality] request failed', { code: error && error.code, message: error && error.message });
